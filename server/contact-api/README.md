...
Your site lives at `https://dk.punds.ch`. Browsers refuse to let an
...
`https://api.dk.punds.ch`. To get one:

1. In your domain's DNS, add an **A record**: `api` → your VPS's IP address.
   (This makes `api.dk.punds.ch` point at the VPS.)
...
Caddy now serves `https://api.dk.punds.ch/contact` with a valid
certificate, proxying to the Node app on port 8080.

Test it:

```bash
curl -X POST https://api.dk.punds.ch/contact \
...
```
VITE_CONTACT_ENDPOINT = https://api.dk.punds.ch/contact
```
...
(`https://dk.punds.ch`, no trailing slash) or the browser blocks the
request (CORS).
