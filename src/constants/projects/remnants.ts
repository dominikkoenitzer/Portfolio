...
export const remnants: Record<Language, LocalizedContent> = {
  en: {
    ...
    architectureDecisions: [
      ...
      "Repoint the extension gallery at Open VSX, replacing the Marketplace dependency cleanly instead of just switching it off.",
    ],
    ...
    qualityAndSecurity: [
      "Private by default: telemetry, crash reporting, and sign-in are gone. Nothing phones home, because there's no phone.",
  ...
  de: {
    ...
    overview:
      "Ich liebe VS Code. Das Chat-Panel, das Agent-Fenster, die Konto-Aufforderung und alles, was still meine Tastenanschläge nach oben meldet, liebe ich nicht. Also habe ich Code - OSS geforkt, den quelloffenen Kern, und jede KI-Oberfläche, jeden Telemetrie-Hook und jede Anmelde-Nerverei entfernt, ohne anzurühren, was wirklich zählt: schnelles Editieren, IntelliSense, das integrierte Terminal, Versionskontrolle und den JavaScript/Node-Debugger. Erweiterungen kommen über Open VSX statt über Microsofts Marketplace. Ausgeliefert wird es als Windows-Installer pro Benutzer, RemnantsUserSetup.exe. Derselbe Editor, ohne den Lärm.",
    roleSummary: "Geforkt, die KI entkernt, den Installer gebaut. Nur ich.",
    problemStatement:
      "VS Code ist ein großartiger Editor, umhüllt von einer wachsenden Schicht aus Cloud-Diensten, Chat-Panels, Agent-Fenstern und Konto-Aufforderungen, um die niemand gebeten hat. Remnants entfernt diese Schicht vollständig, und übrig bleibt der Editor.",
    objectives: [
    ...
    architectureDecisions: [
      ...
      "Die Erweiterungs-Galerie auf Open VSX umlenken und die Marketplace-Abhängigkeit sauber ersetzen, statt sie nur abzuschalten.",
    ],
    implementationHighlights: [
    ...
    qualityAndSecurity: [
      "Privat als Standard: Telemetrie, Absturzberichte und Anmeldung sind weg. Nichts funkt nach Hause, weil es kein Funkgerät gibt.",
    ...
    challengesAndSolutions: [
      {
        challenge:
          "KI und Telemetrie sind tief in den Editor eingewoben. Sie herauszulösen, ohne alles andere zu destabilisieren, war das Schwierige.",
        solution:
          "Ich schneide die Oberflächen auf Produkt- und Contribution-Ebene, sodass sie sich nie registrieren, und lasse die Pfade von Editor, Terminal, Git und Debugger unberührt.",
      },
      {
        challenge:
  ...
  fr: {
    ...
    architectureDecisions: [
      ...
      "Rediriger la galerie d'extensions vers Open VSX et remplacer proprement la dépendance au Marketplace, au lieu de simplement la désactiver.",
    ],
    implementationHighlights: [
    ...
    qualityAndSecurity: [
      "Privé par défaut : la télémétrie, les rapports de plantage et la connexion ont disparu. Rien ne communique vers l'extérieur, parce qu'il n'y a pas de canal.",
    ...
    challengesAndSolutions: [
      {
        challenge:
          "L'IA et la télémétrie sont profondément tissées dans l'éditeur. Les en extraire sans déstabiliser tout le reste, voilà la difficulté.",
        solution:
          "Je coupe les surfaces au niveau du produit et des contributions pour qu'elles ne s'enregistrent jamais, et je laisse intacts les chemins de l'éditeur, du terminal, de Git et du débogueur.",
      },
      {
        challenge:
  ...
  zh: {
    ...
    overview:
      "我喜欢 VS Code。但我不喜欢聊天面板、智能体窗口、账户提示，以及任何悄悄把我的击键上报到上游的东西。于是我分支了 Code - OSS，也就是它的开源内核，精准移除了每一处 AI 界面、每个遥测钩子和每一次登录催促，而对真正重要的东西毫发无损：快速编辑、IntelliSense、集成终端、版本控制以及 JavaScript/Node 调试器。扩展通过 Open VSX 解析，而非 Microsoft 的 Marketplace。它以按用户安装的 Windows 安装程序 RemnantsUserSetup.exe 形式发布。同一个编辑器，没有那些噪音。",
    roleSummary: "我分支它、掏空 AI、做好安装程序。就我一个人。",
    problemStatement:
      "VS Code 是一款出色的编辑器，却被日益增长的云服务、聊天面板、智能体窗口和没人要的账户提示层层包裹。Remnants 彻底移除这一层，剩下的就是编辑器本身。",
    objectives: [
    ...
    architectureDecisions: [
      ...
      "将扩展库重新指向 Open VSX，干净地替换掉对 Marketplace 的依赖，而不是仅仅关掉它。",
    ],
    implementationHighlights: [
    ...
    qualityAndSecurity: [
      "默认私密：遥测、崩溃报告和登录都被拿掉了。什么都不会向外回传，因为根本没有那条线。",
      "作为构建的一部分，对源代码进行完整的 TypeScript 类型检查，与上游工具链保持一致。",
      "诚实的分发：安装程序未签名，按用户安装（无需管理员权限），文档把这一点说清楚，并附带 SmartScreen 说明。",
    ],
    challengesAndSolutions: [
      {
        challenge: "AI 与遥测深深织入编辑器，要把它们抽出来又不动摇其余一切，是这里最难的部分。",
        solution:
          "我在产品层与贡献层切断这些界面，使其永不注册，同时让编辑器、终端、Git 与调试器的路径保持原样。",
      },
      {
