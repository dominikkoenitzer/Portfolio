...
export const oxidize: Record<Language, LocalizedContent> = {
  en: {
    ...
    qualityAndSecurity: [
      ...
      "Files go into quarantine, moved to a timestamped backup folder with their original path preserved, instead of being destroyed outright.",
    ...
    impactPoints: [
      ...
      "Shows destructive tooling can still be safe: every action is previewable and every deletion reversible.",
  ...
  de: {
    ...
    architectureDecisions: [
      "Eine Engine als Rust-Library-Crate mit zwei schlanken Frontends, einer skriptbaren CLI mit JSON-Ausgabe und einem nativen egui-Fenster, damit jedes Feature in beiden gleichzeitig landet.",
    ...
    impactPoints: [
      ...
      "Zeigt, dass destruktives Tooling trotzdem sicher sein kann: jede Aktion vorab einsehbar, jede Löschung umkehrbar.",
  ...
  fr: {
    ...
    qualityAndSecurity: [
      ...
      "Les fichiers partent en quarantaine, déplacés dans un dossier de sauvegarde horodaté avec leur chemin d'origine préservé, plutôt que détruits directement.",
    ...
    impactPoints: [
      ...
      "Montre qu'un outil destructif peut quand même être sûr : chaque action prévisualisable, chaque suppression réversible.",
  ...
  zh: {
    ...
    overview:
      "Windows 自带的卸载总会留下垃圾：孤立的注册表键、残留文件，以及 AppData、ProgramData 和 Program Files 下的空文件夹。所以我用 Rust 直接面向 Win32 API 写了 Oxidize。它读取全部三个卸载注册表视图，运行程序注册的卸载器（MSI 产品用同步的 msiexec），然后扫描注册表和文件系统寻找残留，并为每一项标注 HIGH/MED/LOW 置信度。没有经过验证的 .reg 备份和文件隔离区，任何东西都不会被删除。两个二进制文件，一个引擎：oxidize-cli 用于脚本，oxidize-gui 留给想点鼠标的时候。",
    ...
    qualityAndSecurity: [
      ...
      "文件会进入隔离区，移动到带时间戳、保留原始路径的备份文件夹，而不是直接销毁。",
      "通过 TOKEN_ELEVATION 令牌信息类检测管理员权限（而不是脆弱的写入试探），可选 UAC 重启；单元测试和 clippy 在 CI 中运行。",
    ],
    challengesAndSolutions: [
      {
        challenge:
          "EXE 卸载器常常从 %TEMP% 重启一份自己的副本然后立即退出，退出码因此会谎报卸载是否真正完成。",
        solution: "只把退出码当作参考，通过复查程序的卸载注册表键来确认真正完成。",
      },
      {
        challenge:
          "过于激进的残留扫描器比没有更糟：在 Program Files 或 SOFTWARE\\Microsoft 里错删一项，就可能弄坏整台机器。",
        solution:
    ...
    impactPoints: [
      ...
      "证明破坏性工具照样可以做到安全：每个动作可预览，每次删除可撤销。",
