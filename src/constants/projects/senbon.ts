...
export const senbon: Record<Language, LocalizedContent> = {
  en: {
    ...
    overview:
      "I built Senbon (千本, 'one thousand') because I wanted somewhere to write that wasn't optimized to death. Every entry is a markdown file with frontmatter. There is no database and nobody watching the reader. Next.js App Router and React 19 turn those files into an editorial reading experience: auto-generated table of contents, reading-time estimates, a ⌘K command palette. And robots.ts politely tells search engines and AI crawlers to leave. It's a blog that doesn't want an audience, which is the whole point.",
    ...
    objectives: [
      "Publish long-form entries straight from markdown files, without a CMS or third-party tracking.",
    ...
    architectureDecisions: [
      ...
      "Framer Motion only for entrance animations, with prefers-reduced-motion respected throughout, because calm shouldn't cost anyone a migraine.",
    ],
    ...
    challengesAndSolutions: [
      {
        ...
        solution:
          "One ambient CSS/SVG background and motion gated behind prefers-reduced-motion, so it reads as atmosphere for most people and disappears entirely for anyone who asked for less of it.",
      },
      {
        challenge:
  ...
  de: {
    ...
    overview:
      "Ich habe Senbon (千本, „eintausend“) gebaut, weil ich einen Ort zum Schreiben wollte, der nicht zu Tode optimiert ist. Jeder Eintrag ist eine Markdown-Datei mit Frontmatter. Es gibt keine Datenbank und niemanden, der den Leser beobachtet. Next.js App Router und React 19 machen aus diesen Dateien ein redaktionelles Leseerlebnis: automatisch erzeugtes Inhaltsverzeichnis, Lesezeit-Schätzungen, eine ⌘K-Befehlspalette. Und robots.ts bittet Suchmaschinen und KI-Crawler höflich, draußen zu bleiben. Ein Blog, der kein Publikum will, und genau das ist der Sinn.",
    ...
    objectives: [
      "Langform-Einträge direkt aus Markdown-Dateien veröffentlichen, ohne CMS und ohne Drittanbieter-Tracking.",
    ...
    architectureDecisions: [
      ...
      "Framer Motion nur für Entrance-Animationen, durchgängig unter Beachtung von prefers-reduced-motion, denn Ruhe darf niemandem Kopfschmerzen bereiten.",
    ],
    implementationHighlights: [
    ...
    challengesAndSolutions: [
      {
        ...
        solution:
          "Ein einziger ruhiger CSS/SVG-Hintergrund und Bewegung hinter prefers-reduced-motion: für die meisten ist es Atmosphäre, und wer weniger Bewegung will, bekommt sie gar nicht erst.",
      },
      {
        challenge:
  ...
  fr: {
    ...
    overview:
      "J'ai construit Senbon (千本, « mille ») parce que je voulais un endroit pour écrire qui ne soit pas optimisé à mort. Chaque entrée est un fichier markdown avec frontmatter. Il n'y a pas de base de données, et personne d'extérieur n'observe le lecteur. Next.js App Router et React 19 transforment ces fichiers en une expérience de lecture éditoriale : table des matières auto-générée, estimations du temps de lecture, une palette de commandes ⌘K. Et robots.ts demande poliment aux moteurs de recherche et aux robots d'IA de rester dehors. Un blog qui ne veut pas d'audience, et c'est tout l'intérêt.",
    ...
    objectives: [
      "Publier des entrées longues directement depuis des fichiers markdown, sans CMS ni traçage tiers.",
    ...
    architectureDecisions: [
      ...
      "Framer Motion uniquement pour les animations d'entrée, avec prefers-reduced-motion respecté de bout en bout, parce que le calme ne devrait donner de migraine à personne.",
    ],
    implementationHighlights: [
    ...
    challengesAndSolutions: [
      {
        ...
        solution:
          "Un seul arrière-plan CSS/SVG ambiant et un mouvement conditionné par prefers-reduced-motion : de l'atmosphère pour la plupart des gens, et plus rien du tout pour qui en a demandé moins.",
      },
      {
        challenge:
  ...
  zh: {
    ...
    overview:
      "我做 Senbon（千本，“一千”），是因为想要一个不被优化到死的写作之所。每篇条目都是带 frontmatter 的 Markdown 文件。没有数据库，也没有任何第三方盯着读者。Next.js App Router 与 React 19 把这些文件变成编辑式的阅读体验：自动生成目录、阅读时长估算、一个 ⌘K 命令面板。而 robots.ts 则礼貌地请搜索引擎和 AI 爬虫止步门外。一个不想要观众的博客，而这正是它的意义所在。",
    roleSummary: "设计、构建、上线，都是我一个人。",
    problemStatement:
      "每一款博客工具都在为传播而优化：SEO、订阅源、互动曲线。我想要的恰恰相反，一个私密、亲手打理的思考与写作空间，快速且无障碍，并刻意不进入公开索引。",
    objectives: [
      "直接用纯 Markdown 文件发布长篇条目，不依赖 CMS，也没有第三方追踪。",
    ...
    architectureDecisions: [
      ...
      "Framer Motion 仅用于入场动画，并全程遵循 prefers-reduced-motion，因为宁静不该让任何人头疼。",
    ],
    implementationHighlights: [
    ...
    challengesAndSolutions: [
      {
        ...
        solution: "采用单一的舒缓 CSS/SVG 背景，并把动效放在 prefers-reduced-motion 之后：多数人看到的是氛围，而要求减少动态的人则完全不会遇到它。",
      },
      {
