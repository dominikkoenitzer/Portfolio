...
export const time: Record<Language, LocalizedContent> = {
  en: {
    ...
    architectureDecisions: [
      ...
      "The client keeps the lowest round-trip sample, compensates for RTT/2, and treats that as the accuracy bound, so no single request gets to be the source of truth.",
    ...
    implementationHighlights: [
      ...
      "Time and timezone computed entirely client-side with the built-in Intl APIs, so there is no external time API and no timezone data files.",
    ],
    qualityAndSecurity: [
    ...
    hiringSignals: [
      "Implemented real NTP-style clock discipline with round-trip sampling and offset correction, not a polite wrapper around Date.now().",
  ...
  de: {
    ...
    objectives: [
      ...
      "Die Dimension der Zeit spürbar machen, indem man von einer Sekunde bis zur Unix-Epoche scrollt.",
    ],
    architectureDecisions: [
      ...
      "Der Client behält das Sample mit dem niedrigsten Round-Trip, kompensiert um RTT/2 und behandelt das als Genauigkeitsgrenze, damit keine einzelne Anfrage die Wahrheit sein darf.",
    ...
    implementationHighlights: [
      ...
      "Zeit und Zeitzone werden vollständig client-seitig mit den eingebauten Intl-APIs berechnet, ohne externe Zeit-API und ohne Zeitzonen-Dateien.",
    ],
    qualityAndSecurity: [
      "Watchdogs synchronisieren neu, sobald die Wall-vs-Monotonic-Basislinie springt, damit die angezeigte Genauigkeit die tatsächliche bleibt.",
    ...
    hiringSignals: [
      "Echte NTP-artige Uhren-Disziplin implementiert, mit Round-Trip-Sampling und Offset-Korrektur, kein höflicher Wrapper um Date.now().",
    ...
    impactPoints: [
      ...
      "Verwandelt die Dimension der Zeit durch Scrollen in etwas Spürbares.",
  ...
  fr: {
    ...
    objectives: [
      ...
      "Rendre l'échelle du temps palpable en défilant d'une seconde jusqu'à l'époque Unix.",
    ],
    architectureDecisions: [
      ...
      "Le client garde l'échantillon au plus faible aller-retour, compense de RTT/2 et le traite comme borne de précision, pour qu'aucune requête seule n'ait le droit d'être la vérité.",
    ...
    implementationHighlights: [
      ...
      "Heure et fuseau horaire calculés entièrement côté client avec les API Intl intégrées, sans API de temps externe ni fichier de données de fuseaux.",
    ],
    qualityAndSecurity: [
      "Des chiens de garde resynchronisent dès que la base wall-vs-monotonic saute, si bien que la précision affichée reste celle que j'ai vraiment.",
    ...
    hiringSignals: [
      "Mise en œuvre d'une vraie discipline d'horloge à la manière de NTP, avec échantillonnage aller-retour et correction d'offset, pas un habillage poli de Date.now().",
    ...
    impactPoints: [
      ...
      "Transforme l'échelle du temps en quelque chose que l'on ressent en défilant.",
  ...
  zh: {
    ...
    implementationHighlights: [
      ...
      "时间与时区完全在客户端用内置的 Intl API 计算，不需要外部时间 API，也不需要时区数据文件。",
    ],
    qualityAndSecurity: [
    ...
    hiringSignals: [
      "实现了真正 NTP 式的时钟校准，包括往返采样与偏移校正，而不是对 Date.now() 客客气气的简单封装。",
