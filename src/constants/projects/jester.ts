...
export const jester: Record<Language, LocalizedContent> = {
  en: {
    ...
    qualityAndSecurity: [
      "Safe by default: it asks before throwing away unsaved work, including on sign-out or shutdown, and writes atomically, so a crash can't leave you a corrupted file.",
    ...
    challengesAndSolutions: [
      ...
      },
      {
        ...
        solution:
          "I detect the BOM on open, track encoding and line endings per tab, and write them straight back on save, without any silent \"helpful\" conversions.",
      },
    ],
    hiringSignals: [
      "Native Windows desktop development in C# and WPF on a current .NET 9 stack.",
      "Care where it counts: atomic saves, crash safety, encoding fidelity, not just happy-path features.",
  ...
  de: {
    ...
    description:
      "Jester ist ein nativer Windows-Notepad mit Tab-Bearbeitung, Zeilennummern-Leiste, Find-in-Files und PDF-Export, alles in einer portablen Jester.exe ohne Installer.",
    overview:
      "Ich wollte einen Notepad, der offen ist, bevor ich die Maus loslasse, der aber trotzdem die paar Dinge kann, die Notepad verweigert. Also habe ich einen in C# und WPF auf .NET 9 gebaut: Tabs, eine Zeilennummern-Leiste, Suche über einen ganzen Ordner und sauberer A4-PDF-Export. Er kommt als eine einzige portable Datei mit eingebackener Runtime, also ohne Installer und ohne \"Bitte .NET installieren\"-Dialog.",
    ...
    objectives: [
      "Sofort starten und leicht bleiben, ohne Splash-Screen und ohne Warmlaufen.",
      "Die Features hinzufügen, die zählen (Tabs, Zeilennummern, Find-in-Files, PDF-Export), und den Rest weglassen.",
    ...
    implementationHighlights: [
      ...
      "Find in Files, das mit Filtern und Unterordnern einen ganzen Ordner durchkämmt und per Doppelklick direkt zum Treffer springt.",
    ...
    challengesAndSolutions: [
      ...
      },
      {
        ...
        solution:
          "Ich erkenne das BOM beim Öffnen, verfolge Kodierung und Zeilenenden pro Tab und schreibe sie beim Speichern direkt zurück, ohne stille \"hilfreiche\" Konvertierungen.",
      },
    ],
    hiringSignals: [
      "Native Windows-Desktop-Entwicklung in C# und WPF auf einem aktuellen .NET-9-Stack.",
      "Sorgfalt dort, wo sie zählt: atomare Speicherungen, Absturzsicherheit, Kodierungstreue, nicht nur Happy-Path-Features.",
    ...
    impactPoints: [
      ...
      "Fügt ein paar Power-Features hinzu (Tabs, Suche, PDF-Export), ohne das Gewicht einer vollen IDE.",
  ...
  fr: {
    ...
    objectives: [
      ...
      "Ajouter les fonctions qui comptent (onglets, numéros de ligne, recherche dans les fichiers, export PDF) et laisser tomber le reste.",
    ...
    implementationHighlights: [
      ...
      "Recherche dans les fichiers qui ratisse un dossier entier, filtres et sous-dossiers compris, et saute droit au résultat au double-clic.",
    ...
    challengesAndSolutions: [
      ...
      },
      {
        ...
        solution:
          "Je détecte le BOM à l'ouverture, je suis l'encodage et les fins de ligne par onglet, et je les réécris tels quels à l'enregistrement, sans conversions \"serviables\" silencieuses.",
      },
    ],
    hiringSignals: [
      "Développement desktop Windows natif en C# et WPF sur une pile .NET 9 actuelle.",
      "Du soin là où ça compte : enregistrements atomiques, sûreté face aux plantages, fidélité d'encodage, pas seulement le chemin idéal.",
  ...
  zh: {
    ...
    description:
      "Jester 是一款原生 Windows 记事本，支持标签页编辑、行号边栏、跨文件查找与 PDF 导出，全部装在单个可移植的 Jester.exe 里，无需安装程序。",
    overview:
      "我想要一款在我松开鼠标之前就已经打开的记事本，但它还得会做那几件系统记事本拒绝做的事。于是我用 C# 与 WPF、基于 .NET 9 写了一个：标签页、行号边栏、跨整个文件夹的查找，以及干净的 A4 PDF 导出。它以单个可移植可执行文件发布，运行时已内置，所以没有安装程序，也不会弹出\"请安装 .NET\"。",
    roleSummary: "我一个人设计、构建并发布：WPF 界面和整个 .NET 9 构建。",
    problemStatement:
      "Windows 自带的记事本太简陋，住不进去；而功能完整的编辑器一启动就先去喝杯咖啡。Jester 正是我想要的折中：瞬时启动，外加那几个每天都对得起自己分量的强力功能。",
    objectives: [
      "瞬时启动并保持轻量，没有启动画面，也不用预热。",
      "加入真正重要的功能：标签页、行号、跨文件查找、PDF 导出；其余的一概不加。",
    ...
    implementationHighlights: [
      ...
      "跨文件查找可以带上筛选条件和子文件夹横扫整个文件夹，双击结果即可直接跳转。",
      "PDF 导出只需一个按键，便能把任意笔记变成干净的分页 A4 文档。",
    ],
    qualityAndSecurity: [
      "默认安全：在丢弃未保存的工作前会询问，注销或关机时也一样；写入是原子的，崩溃不会留下损坏的文件。",
    ...
    hiringSignals: [
      ...
      "在关键处用心：原子保存、崩溃安全、编码保真，而不只是顺风顺水的那条路。",
    ...
    impactPoints: [
      ...
      "加入几个真正用得上的强力功能（标签页、搜索、PDF 导出），而没有完整 IDE 的负担。",
