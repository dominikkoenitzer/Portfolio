...
export const portfolio: Record<Language, LocalizedContent> = {
  en: {
    ...
    overview:
      "Most portfolios are a template with the developer's name swapped in. I wanted mine to be the work sample itself. So I built this in React, TypeScript, and Vite: four languages with a hand-rolled i18n layer (no library), JSON-LD structured data so search and AI can actually read it, framer-motion page transitions, smooth scrolling, and a lazy WebGL background.",
    roleSummary:
    ...
    architectureDecisions: [
      "React 19 + TypeScript + Vite with manual code-splitting, so the WebGL background loads lazily and never blocks the critical path.",
    ...
    implementationHighlights: [
      ...
      "An SEO component that emits Person and breadcrumb JSON-LD plus Helmet meta and self-canonical routes.",
    ...
    qualityAndSecurity: [
      "Typecheck and production build are the CI gates, and every push and pull request has to pass both.",
    ...
    impactPoints: [
      ...
      "Proves the craft directly, because the site itself is the engineering sample.",
    ],
  },
  de: {
    tagline:
    ...
    overview:
      "Die meisten Portfolios sind ein Template, in dem nur der Name ausgetauscht wurde. Meines sollte selbst die Arbeitsprobe sein. Also habe ich es in React, TypeScript und Vite gebaut: vier Sprachen mit selbstgebauter i18n-Schicht (ohne Bibliothek), JSON-LD-Strukturdaten, damit Suche und KI es wirklich lesen können, framer-motion-Seitenübergänge, sanftes Scrollen und einen lazy geladenen WebGL-Hintergrund.",
    roleSummary:
      "Entworfen, gebaut, ausgeliefert. Nur ich: Architektur, vier Sprachen, SEO und das visuelle System.",
    problemStatement:
      "Ein Entwickler-Portfolio muss zwei sehr unterschiedliche Leser überzeugen: einen Recruiter, der am Handy überfliegt, und einen Crawler, der nur Markup sieht. Die meisten Seiten wählen eines und zucken beim anderen mit den Schultern. Ich habe für beide gebaut: klare Texte für Menschen, strukturierte Daten für Maschinen, auf einer schnellen, barrierefreien SPA.",
    objectives: [
      "Wirklich mehrsprachig sein (en/de/fr/zh), mit Texten in Muttersprachenqualität statt eines nachträglich angeschraubten Umschalters.",
    ...
    architectureDecisions: [
      "React 19 + TypeScript + Vite mit manuellem Code-Splitting, damit der WebGL-Hintergrund lazy lädt und nie den kritischen Pfad blockiert.",
    ...
    implementationHighlights: [
      ...
      "Eine SEO-Komponente, die Person- und Breadcrumb-JSON-LD sowie Helmet-Meta und selbst-kanonische Routen ausgibt.",
    ...
    impactPoints: [
      ...
      "Beweist das Handwerk direkt, denn die Seite selbst ist die Engineering-Probe.",
    ],
  },
  fr: {
    tagline:
    ...
    overview:
      "La plupart des portfolios sont un template où l'on a juste remplacé le nom. Je voulais que le mien soit l'échantillon lui-même. Je l'ai donc bâti en React, TypeScript et Vite : quatre langues via une couche i18n faite maison (sans bibliothèque), des données structurées JSON-LD pour que la recherche et l'IA puissent vraiment le lire, des transitions de page framer-motion, un défilement fluide et un fond WebGL chargé en lazy.",
    roleSummary:
    ...
    architectureDecisions: [
      "React 19 + TypeScript + Vite avec découpage de code manuel, pour que le fond WebGL se charge en lazy et ne bloque jamais le chemin critique.",
    ...
    implementationHighlights: [
      ...
      "Un composant SEO qui émet du JSON-LD Person et fil d'Ariane, ainsi que des balises Helmet et des routes auto-canoniques.",
    ...
    qualityAndSecurity: [
      "Le typecheck et le build de production sont les garde-fous CI, et chaque push et pull request doit passer les deux.",
    ...
    impactPoints: [
      ...
      "Prouve le savoir-faire directement, puisque le site lui-même est l'échantillon d'ingénierie.",
    ],
  },
  zh: {
    ...
    overview:
      "大多数作品集只是把模板里的名字换成自己的。我希望我的这个本身就是那份工程样本。所以我用 React、TypeScript 和 Vite 把它做了出来：四种语言，基于自研的 i18n 层（不用任何库），配备 JSON-LD 结构化数据，让搜索与 AI 真正读得懂，再加上 framer-motion 页面过渡、平滑滚动，以及惰性加载的 WebGL 背景。",
    roleSummary:
      "设计、构建、上线，全是我一个人：架构、四种语言、SEO 与视觉系统。",
    problemStatement:
      "开发者作品集得说服两类截然不同的读者：在手机上扫一眼的招聘者，以及只看得到标记的爬虫。大多数网站只顾其一，对另一个耸耸肩。我两者都照顾到了：给人看的清晰文案，给机器读的结构化数据，都建在一个快速、可访问的 SPA 之上。",
    objectives: [
    ...
    architectureDecisions: [
      "React 19 + TypeScript + Vite，配合手动代码分割，让 WebGL 背景惰性加载，绝不阻塞关键路径。",
    ...
    implementationHighlights: [
      ...
      "一个 SEO 组件，输出 Person 与面包屑 JSON-LD，以及 Helmet 元标签与自规范化路由。",
