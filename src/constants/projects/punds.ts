...
export const punds: Record<Language, LocalizedContent> = {
  en: {
    ...
    qualityAndSecurity: [
      ...
      "Static output and no server, so there's nothing to attack at runtime.",
    ...
    hiringSignals: [
      ...
      "Restraint where it counts: local state and constants instead of a router and a state library.",
  ...
  de: {
    ...
    description:
      "Punds ist mein Single-Page-Hub im Gewand eines leuchtenden Kathodenstrahl-Terminals: eine Seite, die zu jedem Projekt, jedem Profil und jeder Site verlinkt, die ich online habe.",
    overview:
    ...
    problemStatement:
      "Eine Handvoll verstreuter Projekte und Profile ist mühsam zu teilen; am Ende rasselt man URLs herunter. Punds fasst den ganzen Haufen auf einer Seite zusammen, die unverkennbar meine und leicht weiterzugeben ist.",
    objectives: [
    ...
    qualityAndSecurity: [
      ...
      "Statische Ausgabe und kein Server, also gibt es zur Laufzeit nichts anzugreifen.",
    ...
    hiringSignals: [
      "Frontend-Handwerk, das nicht von der Stange kommt: maßgeschneiderte CSS-Effekte und eine von Grund auf gebaute Canvas-Animation.",
      "Zurückhaltung, wo sie zählt: lokaler State und Konstanten statt Router und State-Library.",
  ...
  fr: {
    ...
    qualityAndSecurity: [
      ...
      "Sortie statique et aucun serveur, donc rien à attaquer à l'exécution.",
    ...
    hiringSignals: [
      ...
      "De la retenue là où ça compte : de l'état local et des constantes plutôt qu'un routeur et une bibliothèque d'état.",
  ...
  zh: {
    ...
    hiringSignals: [
      "不是现成拿来的前端手艺：定制的 CSS 效果与从零构建的 canvas 动画。",
      "在关键处的克制：用本地状态与常量，而不是路由器和状态库。",
